...
const
  ...
  // nmhdr.code = DWORD(DMN_CLOSE, 0)); //nmhdr.hwndFrom = hwndNpp; //nmhdr.idFrom = ctrlIdNpp;
  DMN_DOCK = (DMN_FIRST + 2);
  DMN_FLOAT = (DMN_FIRST + 3);
  // nmhdr.code = DWORD(DMN_XXX, int newContainer);	//nmhdr.hwndFrom = hwndNpp; //nmhdr.idFrom = ctrlIdNpp;
...
{$ELSE}
  ...
  nppPChar = PAnsiChar;
{$ENDIF}
  TNppLang = (L_TXT, L_PHP, L_C, L_CPP, L_CS, L_OBJC, L_JAVA, L_RC, L_HTML,
    L_XML, L_MAKEFILE, L_PASCAL, L_BATCH, L_INI, L_NFO, L_USER, L_ASP, L_SQL,
    L_VB, L_JS, L_CSS, L_PERL, L_PYTHON, L_LUA, L_TEX, L_FORTRAN, L_BASH,
    L_FLASH, L_NSIS, L_TCL, L_LISP, L_SCHEME, L_ASM, L_DIFF, L_PROPS, L_PS,
    L_RUBY, L_SMALLTALK, L_VHDL, L_KIX, L_AU3, L_CAML, L_ADA, L_VERILOG,
    L_MATLAB, L_HASKELL, L_INNO,
...
begin
  ...
  SendMessage(self.NppData.NppHandle, NPPM_GETPLUGINSCONFIGDIR, MAX_PATH, LPARAM(PChar(s)));
...
begin
  if (HWND(sn^.nmhdr.hwndFrom) = self.NppData.NppHandle) and
    (sn^.nmhdr.code = NPPN_TB_MODIFICATION) then
  begin
...
begin
  ...
  if (r) then
    SendMessage(self.NppData.ScintillaMainHandle,
      SciSupport.SCI_GOTOLINE, Line, 0);
  Result := r;
end;
